...
const (
	...
	mlserverContainerMemReqBytes        string = "CONTAINER_MEM_REQ_BYTES"
	defaultMLServerContainerMemReqBytes        = -1
	mlserverMemBufferBytes              string = "MEM_BUFFER_BYTES"
	defaultMLServerMemBufferBytes              = 256 * 1024 * 1024 // 256MB
...
func GetAdapterConfigurationFromEnv(log logr.Logger) (*AdapterConfiguration, error) {
	...
	adapterConfig.MLServerPort = GetEnvInt(runtimePort, defaultRuntimePort, log)
	adapterConfig.MLServerContainerMemReqBytes = GetEnvInt(mlserverContainerMemReqBytes, defaultMLServerContainerMemReqBytes, log)
	adapterConfig.MLServerMemBufferBytes = GetEnvInt(mlserverMemBufferBytes, defaultMLServerMemBufferBytes, log)
	adapterConfig.CapacityInBytes = adapterConfig.MLServerContainerMemReqBytes - adapterConfig.MLServerMemBufferBytes
